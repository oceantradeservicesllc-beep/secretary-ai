...
import { NoteCard, Btn, Spin } from '../components/UI.jsx'
...
export default function TaskDetail() {
  ...
  async function genNote() {
    ...
    setGenAI(true)
    try { const n=await smartNotes(apiKey,task.title,task.category,task.description); if(n) addNote(task.id,n,true) }
  ...
  function del() {
    if (window.confirm(`Delete "${task.title}"?`)) { deleteTask(task.id); go('tasks') }
  }
  ...
  return (
    <div style={{padding:'0 16px 100px'}}>
      ...
      <div style={{background:C.card,border:`0.5px solid ${C.border}`,borderRadius:16,padding:16,marginBottom:14}}>
        ...
        <h2 style={{color:task.isCompleted?C.textMuted:C.text,fontSize:20,fontWeight:700,marginBottom:6,textDecoration:task.isCompleted?'line-through':'none',lineHeight:1.3}}>{task.title}</h2>
        {task.dueDate&&<div style={{color:over?C.danger:C.textMuted,fontSize:13}}>📅 {fmtDate(task.dueDate)}</div>}
      </div>

      {!task.isCompleted&&<div style={{marginBottom:14}}><Btn label="✓  Mark as Complete" color={C.success} onClick={()=>completeTask(task.id)}/></div>}
      ...
      <div style={{background:C.card,border:`0.5px solid ${C.border}`,borderRadius:14,padding:14,marginBottom:14}}>
        ...
        <div style={{display:'flex',gap:8,marginTop:10,alignItems:'flex-end'}}>
          <textarea value={noteText} onChange={e=>setNoteText(e.target.value)} placeholder="Add a note…" rows={2} style={{flex:1,background:C.surface,border:'none',borderRadius:10,padding:'10px 12px',color:C.text,fontSize:13,fontFamily:'Inter,sans-serif',resize:'none',outline:'none'}}/>
      ...
      {task.isCompleted&&(
        <div style={{marginBottom:14}}>
          <Btn label={archiving?'Archiving…':task.isArchivedToOneNote?'✅ Archived to OneNote':'📒  Archive to OneNote'} color="#0078D4" loading={archiving} disabled={task.isArchivedToOneNote} onClick={archive}/>
        </div>
      )}
      ...
      <div style={{background:C.card,border:`0.5px solid ${C.border}`,borderRadius:14,padding:14,marginBottom:14}}>
        ...
        {[['Created',fmtDate(task.createdAt)],['Updated',fmtRel(task.updatedAt)],task.dueDate&&['Due',fmtDate(task.dueDate)],task.completedAt&&['Completed',fmtDate(task.completedAt)]].filter(Boolean).map(([l,v])=>(
      ...
      <button onClick={del} style={{width:'100%',background:'rgba(255,94,94,0.08)',border:'1px solid rgba(255,94,94,0.2)',borderRadius:12,padding:12,color:C.danger,fontSize:14,fontWeight:600,cursor:'pointer',fontFamily:'Inter,sans-serif'}}>🗑 Delete Task</button>
    </div>
  )
}
