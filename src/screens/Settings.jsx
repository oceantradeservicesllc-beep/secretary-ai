...
export default function Settings() {
  const { apiKey, setApiKey, syncing, syncError, syncNow } = useStore()
  ...
  return (
    <div style={{padding:'0 16px 100px'}}>
      ...
      <div style={{background:C.card,border:`0.5px solid ${C.border}`,borderRadius:16,padding:16,marginBottom:16}}>
        <h3 style={{color:C.text,fontSize:15,fontWeight:700,marginBottom:4}}>☁️ Cloud Sync</h3>
        <p style={{color:C.textSec,fontSize:13,marginBottom:12,lineHeight:1.5}}>
          Your tasks sync automatically across all your devices via Supabase.
        </p>
        {syncError && (
          <div style={{background:'rgba(255,94,94,0.1)',border:'1px solid rgba(255,94,94,0.2)',borderRadius:10,padding:'8px 12px',color:C.danger,fontSize:12,marginBottom:10}}>
            ⚠️ {syncError}
          </div>
        )}
        <button onClick={syncNow} disabled={syncing} style={{width:'100%',background:syncing?C.surface:C.msBlue,color:syncing?C.textMuted:'#fff',border:'none',borderRadius:12,padding:12,fontSize:14,fontWeight:600,cursor:syncing?'not-allowed':'pointer',fontFamily:'Inter,sans-serif'}}>
          {syncing ? '⏳ Syncing...' : '🔄 Sync Now'}
        </button>
        <p style={{color:C.textMuted,fontSize:11,marginTop:8,textAlign:'center'}}>
      ...
      <div style={{background:C.card,border:`0.5px solid ${C.border}`,borderRadius:16,padding:16,marginBottom:16}}>
        ...
        <input
          ...
          placeholder="sk-ant-..."
          style={{width:'100%',background:C.surface,border:`1px solid ${C.border}`,borderRadius:10,padding:'11px 14px',color:C.text,fontSize:14,fontFamily:'Inter,sans-serif',outline:'none',marginBottom:10}}
        />
      ...
      {/* How to get key */}
      <div style={{background:C.card,border:`0.5px solid ${C.border}`,borderRadius:16,padding:16,marginBottom:16}}>
        <h3 style={{color:C.text,fontSize:14,fontWeight:700,marginBottom:12}}>How to get your Anthropic API Key</h3>
        {[
          ['1','Go to console.anthropic.com'],
          ['2','Sign up or log in'],
          ['3','Click Settings → API Keys'],
          ['4','Click "Create Key", name it SecretaryAI'],
      ...
      <div style={{background:C.card,border:`0.5px solid ${C.border}`,borderRadius:16,padding:16}}>
        ...
        <div style={{color:C.textSec,fontSize:13,lineHeight:1.8}}>
          ...
          <div>☁️ Synced via Supabase</div>
          <div>🤖 Powered by Claude Sonnet</div>
